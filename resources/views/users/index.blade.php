        ...
        <div class="modal fade" id="editModal{{ $res->id }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('users.update', ['id' => $res->id]) }}" method="post">
                        @csrf
                        <div class="modal-body">
                            <div class="form-group row">
                                <label for="nama" class="col-sm-3 col-form-label">Nama</label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="nama" id="nama"
                                        placeholder="Masukkan nama" value="{{ $res->name }}" autocomplete="off"
                                        required>
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select name="role" id="role" class="form-control" required>
                                        <option value="">-- Pilih Role ---</option>
                                        <option value="admin" {{ $res->role == 'admin' ? 'selected' : '' }}>Admin
                                        </option>
                                        <option value="kasir" {{ $res->role == 'kasir' ? 'selected' : '' }}>Kasir
                                        </option>
                                    </select>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="deleteModal{{ $res->id }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Apakah Anda yakin ingin menghapus pengguna {{ $res->name }}?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                        <form action="{{ route('users.destroy', ['id' => $res->id]) }}" method="post">
                            @csrf
                            @method('DELETE')
